...
<div>
            ...
            <div class="page-header">
                ...
                <div class="page-breadcrumb">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="#" class="breadcrumb-link">Caule</a></li>
                        </ol>
                    </nav>
                </div>
            </div>
        ...
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
            <div class="card">
                <div class="card-header col-md-12 d-flex justify-content-between align-items-center flex-wrap">
                    ...
                    <div class="">
                        <button class="btn btn-sm  mt-1" wire:click='exportExcel'><i class=" text-success fas fa-file"></i> Exportar Excel</button>
                        <button class="btn btn-sm btn-primary mt-1" wire:click='exportCsv'><i class="fas fa-file"></i> Exportar CVS</button>
                        <button class="btn btn-sm btn-primary mt-1" data-toggle="modal" data-target="#caule"><i class="fa fa-plus"></i> Adicionar</button>
                    </div>
                </div>
                <div class="card-body">
                            ...
                            <tbody>
                                @if (isset($caules) and $caules->count() > 0)
                                    @foreach ($caules as $item)
                                        <tr>
                                            <td style="width:10%">
                                                <img class="img-fluid rounded-full" style="width: 5rem;height:5rem; border-radius: 100%" src="{{($item->imagem != null) ? asset('storage/caules/'.$item->imagem): asset('no-image.png')}}" alt="Imagem da caule {{$item->nome}}">
                                            </td>
                                            <td style="">{{$item->nome?? ''}}</td>
                                            <td style="">{{$item->descricao?? ''}}</td>
                                            <td class="text-center">     
                                                <button wire:click='editCaule({{$item->id}})' data-toggle="modal" data-target="#caule" class="btn btn-sm btn-primary mt-1"><i class="fa fa-edit"></i></button>
    ...
    @include('livewire.admin.modals.caule-modal')
</div>
