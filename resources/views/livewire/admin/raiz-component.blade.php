...
<div>
            ...
            <div class="page-header">
                ...
                <div class="page-breadcrumb">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="#" class="breadcrumb-link">Raizes</a></li>
                        </ol>
                    </nav>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-header col-md-12 d-flex justify-content-between align-items-center flex-wrap">
                    ...
                    <div class="">
                        ...
                        <button class="btn btn-sm btn-primary mt-1" data-toggle="modal" data-target="#raiz"><i class="fa fa-plus"></i> Adicionar</button>
                    </div>
                </div>
                <div class="card-body">
                            ...
                            <tbody>
                                @if (isset($raizes) and $raizes->count() > 0)
                                    @foreach ($raizes as $item)
                                        <tr>
                                            <td style="width:10%">
                                                <img class="img-fluid rounded-full" style="width: 5rem;height:5rem; border-radius: 100%" src="{{($item->imagem != null) ? asset('storage/raizes/'.$item->imagem): asset('no-image.png')}}" alt="Imagem da categoria {{$item->nome}}">
                                            </td>
                                            <td style="">{{$item->nome?? ''}}</td>
                                            <td style="">{{$item->descricao?? ''}}</td>
                                            <td class="text-center">     
                                                <button wire:click='editRaiz({{$item->id}})' data-toggle="modal" data-target="#raiz" class="btn btn-sm btn-primary mt-1"><i class="fa fa-edit"></i></button>
    ...
    @include('livewire.admin.modals.raiz-modal')
</div>
