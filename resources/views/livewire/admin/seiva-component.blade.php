...
<div>
            ...
            <div class="page-header">
                ...
                <div class="page-breadcrumb">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="#" class="breadcrumb-link">Seivas</a></li>
                        </ol>
                    </nav>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-header col-md-12 d-flex justify-content-between align-items-center flex-wrap">
                    ...
                    <div class="">
                        ...
                        <button class="btn btn-sm btn-primary mt-1" data-toggle="modal" data-target="#seiva"><i class="fa fa-plus"></i> Adicionar</button>
                    </div>
                </div>
                <div class="card-body">
                            ...
                            <tbody>
                                @if (isset($seivas) and $seivas->count() > 0)
                                    @foreach ($seivas as $item)
                                        <tr>
                                            <td style="width:10%">
                                                <img class="img-fluid rounded-full" style="width: 5rem;height:5rem; border-radius: 100%" src="{{($item->imagem != null) ? asset('storage/seivas/'.$item->imagem): asset('not-found.png')}}" alt="Imagem da seiva {{$item->nome}}">
                                            </td>
                                            <td style="">{{$item->nome?? ''}}</td>
                                            <td style="">{{$item->descricao?? ''}}</td>
                                            <td class="text-center">     
                                                <button wire:click='editSeiva({{$item->id}})' data-toggle="modal" data-target="#seiva" class="btn btn-sm btn-primary mt-1"><i class="fa fa-edit"></i></button>
    ...
    @include('livewire.admin.modals.seiva-modal')
</div>
