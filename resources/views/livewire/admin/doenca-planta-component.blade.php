...
<div>
            ...
            <div class="page-header">
                ...
                <div class="page-breadcrumb">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="#" class="breadcrumb-link">Plantas e Doencas</a></li>
                        </ol>
                    </nav>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-header col-md-12 d-flex justify-content-between align-items-center flex-wrap">
                    ...
                    <div class="">
                        ...
                        <button class="btn btn-sm btn-primary mt-1" data-toggle="modal" data-target="#doencaPlanta"><i class="fa fa-plus"></i> Adicionar</button>
                    </div>
                </div>
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table id="example" class="text-center table table-sm table-striped table-hover" style="width:100%">
                            ...
                            <tbody>
                                @if (isset($doencaPlanta) and $doencaPlanta->count() > 0)
                                    @foreach ($doencaPlanta as $item)
                                        <tr>
                                            <td style="width:10%">
                                                <img class="img-fluid rounded-full" style="width: 5rem;height:5rem; border-radius: 100%" src="{{($item->imagem != null) ? asset('storage/folhas/'.$item->imagem): asset('no-image.png')}}" alt="Imagem {{$item->nome}}">
                                            </td>
                                            <td style="">{{$item->planta_id?? ''}}</td>
                                            <td style="">{{$item->doenca_id?? ''}}</td>

                                            <td class="text-center">     
                                                <button wire:click='editee({{$item->id}})' data-toggle="modal" data-target="#doencaPlanta" class="btn btn-sm btn-primary mt-1"><i class="fa fa-edit"></i></button>
    ...
    @include('livewire.admin.modals.doencaPlanta-modal') 
</div>
