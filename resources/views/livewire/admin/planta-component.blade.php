...
<div>
            ...
            <div class="card">
                <div class="card-header col-md-12 d-flex justify-content-between align-items-center flex-wrap">
                    ...
                    <div class="">
                        ...
                        <button class="btn btn-sm btn-primary mt-1" data-toggle="modal" data-target="#planta"><i class="fa fa-plus"></i> Adicionar</button>
                    </div>
                </div>
                <div class="card-body">
                                ...
                                @if (isset($plantas) and $plantas->count() > 0)
                                    @foreach ($plantas as $item)
                                        <tr>
                                            ...
                                            <td style="width:10%">
                                                <img class="img-fluid rounded-full" style="width: 5rem;height:5rem; border-radius: 100%" src="{{($item->imagem != null) ? asset('storage/plantas/'.$item->imagem): asset('not-found.png')}}" alt="Imagem da categoria {{$item->description}}">
                                            </td>
                                            <td style="">{{$item->planta?? ''}}</td>
                                            <td style="">{{$item->descricao?? ''}}</td>

                                            <td class="text-center">     
                                                <button wire:click='editPlanta({{$item->id}})' data-toggle="modal" data-target="#planta" class="btn btn-sm btn-primary mt-1"><i class="fa fa-edit"></i></button>
    ...
    @include('livewire.admin.modals.planta-modal')
</div>
